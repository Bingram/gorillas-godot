...
const gravity: float = 9.8 # earth gravity 9.8m/s
var time: float = 0.0
...
func _process(delta) -> void:
	...
	if Input.is_action_just_pressed("Launch"):
		LaunchProjectile(global_position, Vector2(1, 0), 150, 60)
	...
	if is_launch:
		...
		if not is_on_floor():
			...
			$banana.position.y = -z_axis # Move only the projectile along the y axis based on the simulated z-axis
